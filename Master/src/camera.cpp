...
namespace {
    ...
    void update_cam_pos(camera_t &cam, GLFWwindow *win, float dt, bool inRoom) {
        ...
        if (inRoom){
            if (glfwGetKey(win, GLFW_KEY_W) == GLFW_PRESS) {
                cam.pos += glm::vec3(front.x,0,front.z) * step;
                if(cam.pos.x > 10) cam.pos.x = 10;
                if(cam.pos.x < -10) cam.pos.x = -10;
                if(cam.pos.z > 10) cam.pos.z = 10;
                if(cam.pos.z < -10) cam.pos.z = -10;
            }
...
void update_camera(camera_t &cam, GLFWwindow *window, float dt, bool inRoom) {
    update_cam_angles(cam, window, dt);
    update_cam_pos(cam, window, dt, inRoom);
}
